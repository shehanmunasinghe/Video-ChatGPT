...
def parse_args():
    ...
    parser.add_argument('--mapper', help='path to mapper', required=True)
...
def eval_model(args):
    ...
    mappings = {}
    for entry in maps:
        video_id = int(entry.split()[1].strip('vid'))
        mappings[video_id] = entry.split()[0]
    ...
    for sample in tqdm(gt_contents):
        ...
        video_name = mappings[video_id]
        output_set = sample

        try:
            # loading video file            
            video_path = os.path.join(args.video_dir, f"{video_name}.avi")
