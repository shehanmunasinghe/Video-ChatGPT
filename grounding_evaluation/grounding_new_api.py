...
class Tracker_with_GroundingDINO:
    ...
    def run_on_frame(self, frame, save_at_fps=5):
        ...
        if self.output_video_path:
            if not self.result_writer_initialized:
                ...
                self.result_saver.writer = cv2.VideoWriter(self.output_video_path, cv2.VideoWriter_fourcc(*'MP4V'), save_at_fps, (w, h))
    ...
    def save_result_on_frame(self, prob, frame_name, need_resize, shape, image_np, prompts):
        if self.output_video_path:
            self.result_saver.save_mask(prob, frame_name, need_resize=need_resize, shape=shape, image_np=image_np, prompts=prompts)
